import pytest
import time
from src.message import message_send_v2, message_react_v1, message_senddm_v1
...
def test_success_channel(create_channel):
    ...
    assert message_react_v1(token, m_id, 1) == {}
...
def test_non_auth_user_channel(create_channel):
    ...
    with pytest.raises(AccessError):
        message_react_v1(token2['token'], m_id, 1)

def test_message_already_reacted(create_channel):
    token, _, m_id = create_channel
...
def test_invalid_auth_user_dm(create_dm):
    ...
    with pytest.raises(AccessError):
        message_react_v1(token3['token'], m_id, 1)
...
def test_invalid_message(create_channel):
    token, _, m_id = create_channel
    with pytest.raises(InputError):
        message_react_v1(token, m_id + 120, 1)

def test_invalid_react(create_channel):
    token, _, m_id = create_channel
    with pytest.raises(InputError):
        message_react_v1(token, m_id, 2)